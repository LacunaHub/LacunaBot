...
module.exports = async (self, server, interaction) => {
    ...
    if (!mention) {
        await interaction.reply({ content: `${self._emojis.ERROR} | ${self.translator.format(locale.violations.texts.user_not_found, `**${interaction.member.displayName}**`)}`, ephemeral: true })
    ...
    if (!violator || !violator.violations.length) {
        await interaction.reply({ content: `${self._emojis.ERROR} | ${self.translator.format(locale.violations.texts.no_violations, `**${interaction.member.displayName}**`)}`, ephemeral: true })
