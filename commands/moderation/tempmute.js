...
const ms = require('ms')
const Tempmute = require('../../internals/structures/TemporaryMute')
...
const execute = async (self, server, message, args) => {
    ...
    if (!member) {
        await message.channel.send(`${self._emojis.ERROR} | ${self.translator.format(locale.tempmute.texts.user_not_found, `**${message.author.username}**`)}`)

        return false
    }

    if (member.hasPermission("MANAGE_ROLES")) {
        await message.channel.send(`${self._emojis.ERROR} | ${self.translator.format(locale.tempmute.texts.user_has_moder_permission, `**${message.author.username}**`)}`)

        return false
    }

    if (!member.bannable) {
        await message.channel.send(`${self._emojis.ERROR} | ${self.translator.format(locale.tempmute.texts.cant_mute_user, `**${message.author.username}**`)}`)

        return false
    }

    if (!timer) {
        await message.channel.send(`${self._emojis.ERROR} | ${self.translator.format(locale.tempmute.texts.no_timer_argument, `**${message.author.username}**`)}`)

        return false
    }

    if (!ms(timer)) {
        await message.channel.send(`${self._emojis.ERROR} | ${self.translator.format(locale.tempmute.texts.invalid_timer_argument, `**${message.author.username}**`)}`)
    ...
    if (member.roles.cache.has(mute_role) || tempmute) {
        await message.channel.send(`${self._emojis.ERROR} | ${self.translator.format(locale.tempmute.texts.user_already_muted, `**${message.author.username}**`)}`)
    ...
    const dm_message = new MessageEmbed()
        ...
        .setFooter(null, message.guild.iconURL())
        .setTimestamp()
        .setColor(0xF04747)
    ...
    const case_log_message = new MessageEmbed()
        ...
        .setThumbnail(images.MUTE_ADD_TEMP)
        .setTimestamp()
        .setColor(0xF04747)
...
module.exports = {
    fn: execute,
    name: 'tempmute',
    description: 'commands.tempmute.description',
    group: 'moderation',
    guild_only: true,
