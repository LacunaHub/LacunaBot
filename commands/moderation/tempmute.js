...
const execute = async (self, server, message, args) => {
    ...
    if (!timer) {
        await message.reply(`${self._emojis.ERROR} | ${self.translator.format(locale.tempmute.texts.no_timer_argument, `**${message.author.username}**`)}`, { allowedMentions: { repliedUser: false } })

        return false
    }

    if (!ms(timer)) {
        await message.reply(`${self._emojis.ERROR} | ${self.translator.format(locale.tempmute.texts.invalid_timer_argument, `**${message.author.username}**`)}`, { allowedMentions: { repliedUser: false } })
