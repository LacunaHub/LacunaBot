...
const execute = async (self, server, message, args) => {
    ...
    if (!volume || isNaN(volume)) {
        await message.channel.send(`${self._emojis.ERROR} | ${self.translator.format(locale.volume.texts.invalid_volume, `**${message.author.username}**`)}`)

        return false
    }

    if (volume < 1 || volume > 100) {
        await message.channel.send(`${self._emojis.ERROR} | ${self.translator.format(locale.volume.texts.volume_diapason, `**${message.author.username}**`)}`)
