...
*/
module.exports = async (self, server, before, sticker) => {
    if (server.moderation.logs.types.sticker_update.active) {
        const locale = self.translator.locale(server.locale).modules

        const log = sticker.guild.channels.cache.get(server.moderation.logs.types.sticker_update.channel_id)
        ...
        if (is_ok) {
            ...
            if (!webhook) {
                ...
                try {
                    webhook = await log.createWebhook(`${self.user.username}`, { avatar: self.user.displayAvatarURL(), reason: self.translator.format(locale.logs.common.webhook_create_reason, locale.logs.sticker_update.title) })
            ...
            await self.emit('moduleExecution', { module: 'Logs: Sticker Update', guild: { id: sticker.guild.id, name: sticker.guild.name }, target: { id: sticker.id, name: sticker.name } })
