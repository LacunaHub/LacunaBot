...
module.exports = async (self, server, sticker) => {
    if (server.moderation.logs.types.sticker_create.active) {
        ...
        const log = sticker.guild.channels.cache.get(server.moderation.logs.types.sticker_create.channel_id)
        ...
        if (is_ok) {
            ...
            await self.emit('moduleExecution', { module: 'Logs: Sticker Create', guild: { id: sticker.guild.id, name: sticker.guild.name }, target: { id: sticker.id, name: sticker.name } })
