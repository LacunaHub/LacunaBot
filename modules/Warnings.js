...
class Warnings {
    ...
    static async add(self, server, message, options) {
        ...
        if (penalty) {
            ...
            if (send_message) {
                const replacer = new Replacer(self, null, { message: message, guild: message.guild, member: target })
     ...
     * @param {string} options.reason
     */
    static async addInteracted(self, server, interaction, options) {
        const target = options.target, executor = options.executor, reason = options.reason
        ...
        if (!violator) {
            ...
            await self.db.servers.update({ _id: interaction.guild.id, 'moderation.warnings.violators.user_id': target.id }, {
                $push: {
                    'moderation.warnings.violators.$.violations': {
                        id: id.simple(5),
                        timestamp: timestamp,
        ...
        if (penalty) {
            ...
            if (ban && (!mute && !kick)) {
                if (penalty.duration) {
                    ...
                    await interaction.guild.members.ban(target.user.id, { reason: 'Автомодер: Предупреждение' }).catch(self.logger.error)
                }
            }
            ...
            if (mute && (!ban && !kick)) {
                ...
                if (mute_role && !tempmute && !mute_role.members.has(target.user.id)) {
                    if (penalty.duration) {
                        ...
                        if (server.moderation.roles.on_mute.remove_all_roles) {
                            const current_roles = target.roles.cache.filter(r => r.editable && r.id != interaction.guild.id).map(r => r.id)
            ...
            if (edit_roles && (!ban && !kick)) {
                if (penalty?.add_roles?.length) {
                    const editable = interaction.guild.roles.cache.filter(r => r.editable && penalty.add_roles.includes(r.id))
                ...
                if (penalty?.remove_roles?.length) {
                    const editable = interaction.guild.roles.cache.filter(r => r.editable && penalty.remove_roles.includes(r.id))
            ...
            if (send_message) {
                const replacer = new Replacer(self, null, { guild: interaction.guild, member: target })
                const content = await replacer.replaceTemplateMessage(penalty.message)

                await interaction.channel.send(content).catch(self.logger.error)
            }
