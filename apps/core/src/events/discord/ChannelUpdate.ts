...
const handler = async (self: Lacuna, before: DMChannel | GuildChannel, channel: DMChannel | GuildChannel) => {
    ...
    if (
        ...
        !before.permissionOverwrites.cache.equals(channel.permissionOverwrites.cache)
    ) {
        ...
        if (autoVoice && child && autoVoice?.moderator_roles?.length) {
            ...
            const roles: string[] = autoVoice.moderator_roles.filter(mr =>
                channel.guild.roles.cache.some(r => r.editable && r.id === mr)
            )
