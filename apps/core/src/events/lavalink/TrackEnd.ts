...
import { getTrackSourceByUrl } from '@/internals/utility/Utils.js'
...
const handler = async (self: Lacuna, player: Player) => {
    ...
    if (message && message.embeds[0]) {
        ...
        const embed = new EmbedBuilder(message.embeds[0] as any).setTitle(`${track.info.author} - ${track.info.title}`)

        embed.data.fields![0]!.value = track.info.isStream
            ? '♾️'
            : // @ts-expect-error
        ...
        if (embed.data.footer?.text)
            embed.setFooter({ text: embed.data.footer.text.replace(/\s[\w\W]+/i, ` ${track.requester}`) })
        if (track.info.artworkUrl) embed.setThumbnail(track.info.artworkUrl)

        try {
            if (player.state === 'CONNECTED') {
                await message.delete()
                const playerMessage = await (message.channel as GuildTextBasedChannel).send({
                    embeds: [embed],
            ...
            self.logger.error({
                module: 'MusicTrackEnd',
                action: 'RecreatePlayerMessage',
                err,
