...
import { validateTemplateMessage } from './ValidateTemplateMessage.js'

export function validateCustomCommand(data: ServerModulesCustomCommand) {
    ...
    if (
        ...
        data.options.every(v => Object.values(ServerModulesCustomCommandOptions).includes(v))
    )
...
export function validateCustomBehaviorScripts(scripts: ServerModulesCustomCommandScript[]) {
    ...
    for (let script of scripts) {
        ...
        if (
            typeof script.language !== 'number' ||
            !Object.values(ServerModulesCustomCommandScriptLanguages).includes(script.language)
        )
            continue
...
export function validateCustomBehaviorComponents(components: ServerModulesCustomCommandComponent[]) {
    ...
    for (let component of components) {
        if (
            ...
            !Object.values(ServerModulesCustomCommandComponentTypes).includes(component.type)
        )
            continue
        ...
        if ('condition' in component) {
            ...
            if (
                ...
                !Object.values(ServerModulesCustomCommandComponentConditionCompareValuesOperators).includes(
                    compare_values.operator
                )
            )
                continue
            ...
            if (
                ...
                !compare_values.options.every(v =>
                    Object.values(ServerModulesCustomCommandComponentConditionCompareValuesOptions).includes(v)
                )
            )
                continue

            const falseReply =
                'false_reply' in compare_values ? validateTemplateMessage(compare_values.false_reply!) : null
        ...
        if ('action' in component) {
            ...
            if (isObject(action.modify_roles)) {
                if (
                    !Array.isArray(action.modify_roles!.add) ||
                    !action.modify_roles!.add.every(v => typeof v === 'string')
                )
                    continue
                if (
                    !Array.isArray(action.modify_roles!.remove) ||
                    !action.modify_roles!.remove.every(v => typeof v === 'string')
                )
                    continue
            ...
            if (isObject(action.modify_wallet)) {
                ...
                if (typeof action.modify_wallet!.currency_id !== 'string' && action.modify_wallet!.currency_id !== null)
                    continue
                if (typeof action.modify_wallet!.user_id !== 'string' && action.modify_wallet!.user_id !== null)
                    continue
            ...
            if (isObject(action.overwrite_channel_permissions)) {
                ...
                if (
                    ...
                    !Object.values(action.overwrite_channel_permissions!.permissions).every(
            ...
            if (isObject(action.reply)) {
                if (
                    ...
                    !action.reply!.options.every(v =>
                        Object.values(ServerModulesCustomCommandComponentActionReplyOptions).includes(v)
                    )
                )
                    continue
            ...
            if (isObject(action.send_message)) {
                if (
                    ...
                    !action.send_message!.options.every(v =>
                        Object.values(ServerModulesCustomCommandComponentActionSendMessageOptions).includes(v)
                    )
                )
                    continue
                if (
                    typeof action.send_message!.format !== 'string' ||
                    !Object.values(ServerModulesCustomCommandComponentActionSendMessageFormats).includes(
                        action.send_message!.format
                    )
                )
                    continue
