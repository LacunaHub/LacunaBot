...
import {
    ...
    getRepository,
    getRepositoryTree
} from '@/api/utility/GitHubAPI.js'
import database from '@/database/index.js'
import { type ServerModulesAutomation, type ServerModulesCustomCommand } from '@/database/schemas/Servers.js'
import { isObject, parseJSON } from '@/internals/utility/Utils.js'
...
export default async function getPlugin(ctx: Context) {
    ...
    ctx.body = {
        ...
        description_l10ns: Object.assign(
            {},
            readmeContentLocales
                .filter(v => languages.some(vv => v.locale === vv.code))
                .map(v => ({ [v.locale!]: v.content }))
        ),
