...
export default async (self: Lacuna, server: ServerDocument, interaction: ChatInputCommandInteraction<'cached'>) => {
    ...
    if (server.moderation.case_log.types.BAN_ADD.active) {
        const replacer = new Replacer({ guild: interaction.guild, member: mention }),
            messagePayload = await replacer.replaceTemplateMessage(
                server.moderation.case_log.types.BAN_ADD.dm_message,
                { penalty: { reason } }
            )
        ...
        try {
            ...
            self.logger.error({ module: 'BanCommand', action: 'SendDirectMessage', err, guildId: interaction.guildId })
        }
    }
    ...
    if (duration) {
        ...
        try {
            ...
            self.logger.error({ module: 'BanCommand', action: 'Ban', err, guildId: interaction.guildId })
        }
    }
