...
export default async function updateInteractiveMessage(ctx: Context) {
    ...
    try {
        switch (method) {
            case 'create':
                response = await createInteractiveMessage(server, data)
                break

            case 'update':
                response = await setInteractiveMessage(server, data)
                break

            case 'delete':
                response = await deleteInteractiveMessage(server, data)
                break
...
export async function setInteractiveMessage(server: ServerDocument, data: ServerModulesInteractiveMessage) {
    ...
    if (hasChanges) {
        try {
            await DiscordUtils.rest.patch(DiscordUtils.restRoutes.channelMessage(im.channel_id, im.id), {
                body: message
            })
...
export async function deleteInteractiveMessage(server: ServerDocument, data: { id: string }) {
    ...
    try {
        await DiscordUtils.rest.delete(DiscordUtils.restRoutes.channelMessage(im.channel_id, im.id))
    } catch (err) {}
