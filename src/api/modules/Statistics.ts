...
export function createCollectionSchedule() {
    ...
    const job = scheduleJob(rule, async () => {
        const stats = await serverClient.broadcastEval((self: Lacuna) => {
            return {
        ...
        if (process.env.NODE_ENV !== 'development') await updateStatsOnListings(totalGuilds)
    })
...
export async function updateStatsOnListings(guilds: number) {
    try {
...
export default { createCollectionSchedule, updateStatsOnListings }
