...
export function validateTemplateMessage(template: ServerMessageTemplate) {
    ...
    if (isObject(template.embed.author)) {
        ...
        if (typeof template.embed.author.url === 'string' || template.embed.author.url === null) message.embed.author.url = template.embed.author.url
    }

    if (typeof template.embed.color === 'string' || template.embed.color === null) message.embed.color = template.embed.color
    if (typeof template.embed.description === 'string' || template.embed.description === null) message.embed.description = template.embed.description

    if (Array.isArray(template.embed.fields)) {
        const fields = []

        for (const field of template.embed.fields) {
            if (typeof field.name === 'string' && typeof field.value === 'string' && (typeof field.inline === 'boolean' || field.inline === null))
                fields.push(field)
        }
    ...
    if (isObject(template.embed.image)) {
        if (typeof template.embed.image.url === 'string' || template.embed.image.url === null) message.embed.image.url = template.embed.image.url
    }
    ...
    if (typeof template.embed.timestamp === 'string' || template.embed.timestamp === null) message.embed.timestamp = template.embed.timestamp
    if (typeof template.embed.title === 'string' || template.embed.title === null) message.embed.title = template.embed.title
