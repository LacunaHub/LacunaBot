...
export default class Automation {
    ...
    public tasks: ServerModulesAutomation[]
    ...
    constructor(self: Lacuna, server: ServerDocument, tasks: ServerModulesAutomation[], signal: AutomationSignal) {
        this.self = self
    ...
    async execute() {
        ...
        for (const automation of this.tasks) {
            if (automation.components.length > 1 && automation.components.some(i => i.action?.type === 'EXECUTE_CODE')) {
                automation.components = [automation.components.find(i => i.action?.type === 'EXECUTE_CODE')]
            }

            for (const component of automation.components) {
                ...
                if (component.type === 'ACTION') {
                    ...
                    if (action.type === 'EXECUTE_CODE' && this.server.premium.available) {
                        ...
                        const functions = {
                            ...
                            modifyUserWallet: async (userId: string, amount: number, currencyId: string = 'DEFAULT') => {
                                ...
                                const userWallet = await this.self.db.users.fetchWallet(user, interaction.guildId)
                    ...
                    if (action.type === 'REPLY') {
                        const index = automation.components.filter(i => i.type === 'ACTION' && i.action.type === action.type).indexOf(component)
                        const { reply } = action
                    ...
                    if (action.type === 'MODIFY_WALLET') {
                        ...
                        if (member) {
                            ...
                            const userWallet = await this.self.db.users.fetchWallet(user, this.signal.guild.id)
        ...
        this.self.emit('moduleExecution', {
            module: 'Automation',
            category: snakeToPascalCase(this.tasks.at(0).trigger),
    ...
    static async handleEvent(
        eventName: ServerModulesAutomationTrigger,
        self: Lacuna,
        server: ServerDocument,
        signal: AutomationSignal,
        overwriteSignalProps: Record<string, any> = {}
    ) {
        const tasks = server.modules.automation
        ...
        if (tasks.length) {
            ...
            const shouldOverwriteProps = !!Object.keys(overwriteSignalProps).length
            if (shouldOverwriteProps) signal = Object.create(signal)

            for (const prop of Object.keys(overwriteSignalProps)) {
                signal[prop] = overwriteSignalProps[prop]
            }

            const am = new Automation(self, server, tasks, signal)
            await am.execute()
        }
    }
}
