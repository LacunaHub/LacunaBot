    ...
    if (upper_percent >= config.percentage_of_caps && message.content.length >= config.minimum_content_length) {
        ...
        if (ban && !mute && !kick && message.member.bannable) {
            if (config.ban_timeout) {
                ...
                try {
                    ...
                    await self.logger.handleError({ module: 'AntiCaps', action: 'Ban', error: err, guild_id: message.guildId })
                }
            }
        ...
        if (mute && !ban && !kick && message.member.moderatable) {
            ...
            try {
                ...
                await self.logger.handleError({ module: 'AntiCaps', action: 'DisableCommunication', error: err, guild_id: message.guildId })
            }
            ...
            if (server.moderation.mutes.rar) {
                ...
                try {
                    ...
                    await self.logger.handleError({ module: 'AntiCaps', action: 'RemoveAllRoles', error: err, guild_id: message.guildId })
                }
            }
        }
        ...
        if (kick && !ban && !mute && message.member.kickable) {
            try {
                ...
                await self.logger.handleError({ module: 'AntiCaps', action: 'Kick', error: err, guild_id: message.guildId })
            }
            ...
            if (config.modify_roles?.add?.length) {
                ...
                if (editable.size) {
                    try {
                        ...
                        await self.logger.handleError({ module: 'AntiCaps', action: 'ModifyRolesAdd', error: err, guild_id: message.guildId })
                    }
                }
            }
            ...
            if (config.modify_roles?.remove?.length) {
                ...
                if (editable.size) {
                    try {
                        ...
                        await self.logger.handleError({ module: 'AntiCaps', action: 'ModifyRolesRemove', error: err, guild_id: message.guildId })
                    }
                }
            }
        }
        ...
        if (send_message && (config.send_message.content || config.send_message.embed.active)) {
            ...
            try {
                ...
                await self.logger.handleError({ module: 'AntiCaps', action: 'SendMessage', error: err, guild_id: message.guildId })
            }
        }
        ...
        if (delete_message) {
            if (message.deletable) {
                try {
                    ...
                    await self.logger.handleError({ module: 'AntiCaps', action: 'DeleteMessage', error: err, guild_id: message.guildId })
                }
            }
        }
