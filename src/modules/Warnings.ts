import { BaseGuildTextChannel, CommandInteraction, GuildMember, Message, MessageEmbed } from 'discord.js'
...
export async function addWarn(self: Lacuna, server: ServerDocument, signal: Message | CommandInteraction, options: WarnOptions) {
    ...
    const case_log = signal.guild.channels.cache.get(server.moderation.case_log.channel_id) as BaseGuildTextChannel
    ...
    if (!violator) {
        await self.db.servers.updateOne({ _id: signal.guild.id }, {
            $push: {
        ...
        await self.db.servers.updateOne({ _id: signal.guild.id, 'moderation.warnings.violators.user_id': target.id }, {
            $push: {
            ...
            if (mute_role && !tempmute && !mute_role.members.has(target.user.id)) {
                if (penalty.duration) {
                    ...
                    if (server.moderation.roles.on_mute.remove_all_roles) {
                        ...
                        const strict_roles: string[] = [...server.moderation.roles.on_mute.strict_roles.filter(r => current_roles.includes(r)), ...target.roles.cache.filter(r => !r.editable).map(r => r.id)]
