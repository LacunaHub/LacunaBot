            ...
            if (!webhook) {
                ...
                try {
                    webhook = await log.createWebhook(`${self.user.username}`, {
                        ...
                        reason: self.translator.format(locale.logs.common.webhook_create_reason, locale.logs.guild_member_update.title)
                    })
                } catch (err) {
                    return false
                }
                ...
                const embed = new MessageEmbed()
                    ...
                    .setDescription(
                        self.translator.format(
                            locale.logs.guild_member_update.template,
                            `**${executor?.tag ?? locale.logs.common.unknown_initiator}**`,
                            self.translator.format(locale.logs.guild_member_update.nickname_update, `**${member.user.tag}** (${member.id})`)
                        )
                    )
