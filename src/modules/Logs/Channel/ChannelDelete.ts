            ...
            if (!webhook) {
                ...
                try {
                    webhook = await log.createWebhook(`${self.user.username}`, {
                        ...
                        reason: self.translator.format(locale.logs.common.webhook_create_reason, locale.logs.channel_delete.title)
                    })
                } catch (err) {
                    return false
                }
            ...
            const embed = new MessageEmbed()
                ...
                .setDescription(
                    self.translator.format(
                        locale.logs.channel_delete.template,
                        `**${executor?.tag ?? locale.logs.common.unknown_initiator}**`,
                        `${locale.logs.channel_create.types[channel.type] ?? locale.logs.channel_create.types.UNKNOWN} **${channel.name}**`
                    )
                )
