...
export default async function(self: Lacuna, server: ServerDocument, channel: GuildChannel): Promise<boolean> {
    if (server.moderation.logs.types.channel_delete.active) {
        ...
        if (is_ok) {
            ...
            const audit = channel.guild.me.permissions.has(self.PERMISSIONS_FLAGS.VIEW_AUDIT_LOG) ? await channel.guild.fetchAuditLogs({ limit: 1, type: 'CHANNEL_DELETE' }) : null
            ...
            const embed = new MessageEmbed()
                ...
                .setDescription(self.translator.format(locale.logs.channel_delete.template, `**${executor?.tag ?? locale.logs.common.unknown_initiator}**`, `${locale.logs.channel_create.types[channel.type] ?? locale.logs.channel_create.types.UNKNOWN} **${channel.name}**`))
            ...
            self.emit('moduleExecution', { module: 'Logs: Channel Delete', guild: { id: channel.guild.id, name: channel.guild.name }, target: { id: channel.id, name: channel.name } })
