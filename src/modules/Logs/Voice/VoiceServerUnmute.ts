...
export default async function(self: Lacuna, server: ServerDocument, state: VoiceState): Promise<boolean> {
    if (server.moderation.logs.types.voice_server_unmute.active) {
        ...
        const log = state.guild.channels.cache.get(server.moderation.logs.types.voice_server_unmute.channel_id) as BaseGuildTextChannel
        ...
        if (is_ok) {
            ...
            self.emit('moduleExecution', { module: 'Logs: Voice Server Unmute', guild: { id: state.guild.id, name: state.guild.name }, target: { id: state.member.id, name: state.member.user.tag } })
