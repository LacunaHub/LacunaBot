...
export default async (self: Lacuna, server: ServerDocument, interaction: CommandInteraction) => {
    ...
    if (!player.queue.previous) {
        await interaction.reply({ content: `${self._emojis.ERROR} | ${self.translator.format(locale.previous.texts.no_previous_track, `**${(interaction.member as any).displayName}**`)}`, ephemeral: true })
