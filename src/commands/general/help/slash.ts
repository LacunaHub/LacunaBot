...
export default async (self: Lacuna, server: ServerDocument, interaction: CommandInteraction) => {
    ...
    if (!command_name) {
        ...
        if (!command && !custom) {
            await interaction.reply({ content: `${self._emojis.ERROR} | ${self.translator.format(locale.commands.help.texts.command_not_found, `**${(interaction.member as GuildMember).displayName}**`)}`, ephemeral: true })
        ...
        if (command) {
            ...
            if (command.options?.filter(opt => !['SUB_COMMAND', 'SUB_COMMAND_GROUP'].includes(opt.type))?.length) {
                ...
                const args = command.options.map(opt => {
