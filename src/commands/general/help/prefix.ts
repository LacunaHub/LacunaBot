...
export default async (self: Lacuna, server: ServerDocument, message: Message) => {
    ...
    const command_name: string = message['args'][0]
    ...
    if (!command_name) {
        ...
        const components = new MessageActionRow().addComponents(
            new MessageButton().setStyle('LINK').setLabel(locale.commands.help.texts.links.website).setURL(`https://www.voidlacuna.ru/guilds/${message.guild.id}`),
