...
export default async (self: Lacuna, server: ServerDocument, interaction: ChatInputCommandInteraction) => {
    ...
    const total_guilds = (await self.cluster.fetchClientValues('guilds.cache.size')) as number[]
    const total_users = self.guilds.cache.reduce((x, y) => x + y.memberCount, 0)
    const cached_users = (await self.cluster.fetchClientValues('users.cache.size')) as number[]
    ...
    const embed = new EmbedBuilder()
        .addFields([
            ...
            { name: t('commands.about.text_shard_count'), value: `${self.cluster.count}`, inline: true },
