...
export default async (self: Lacuna, server: ServerDocument, message: Message) => {
    ...
    if (sorting == 1) {
        ...
        const activities = (await self.db.users.find({ 'activities.levels.guild_id': message.guildId })).map(i => ({
    ...
    if (sorting == 2) {
        ...
        const activities = (await self.db.users.find({ 'activities.wallets.guild_id': message.guildId })).map(i => ({
            user: { id: i._id, ...i.user },
