...
export default async (self: Lacuna, server: ServerDocument, message: Message) => {
    ...
    const embed = new MessageEmbed()
        ...
        .addField(`${locale.user.texts.roles} [${mention.roles.cache.filter(r => r.id != message.guild.id).size}]`, mention.roles.cache.filter(r => r.id != message.guild.id).map(role => `<@&${role.id}>`).join(' ') || '-')
