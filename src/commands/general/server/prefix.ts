...
export default async (self: Lacuna, server: ServerDocument, message: Message) => {
    ...
    const embed = new MessageEmbed()
        ...
        .addField(
            ...
            `${message.guild.channels.cache.filter(c => c.type == 'GUILD_TEXT').size} ${locale.server.texts.channels.text}\n` +
                `${message.guild.channels.cache.filter(c => c.type == 'GUILD_VOICE').size} ${locale.server.texts.channels.voice}`,
            true
        )
