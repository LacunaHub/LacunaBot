...
export async function setWalletBalancePrefix(self: Lacuna, server: ServerDocument, message: Message) {
    ...
    await message.reply({
        content: `${self._emojis.OK} | ${self.translator.format(reply, `**${message.member.displayName}**`, `**${mention.displayName}**`, `**${amount}${currency_symbol}**`)}`
    })
...
export async function resetWalletPrefix(self: Lacuna, server: ServerDocument, message: Message) {
    ...
    if (!activities.length) {
        await message.reply({
            content: `${self._emojis.ERROR} | ${self.translator.format(locale.activities['reset-all-wallets'].texts.nothing_to_reset, `**${message.member.displayName}**`)}`
        })
    ...
    if (member_id == 'all') {
        ...
        collector.on('collect', async i => {
            ...
            switch (i.customId) {
                ...
                case 'cancel':
                    await _message.edit({
                        content: `${self._emojis.OK} | ${self.translator.format(locale.activities['reset-all-wallets'].texts.canceled, `**${message.member.displayName}**`)}`,
                        components: []
                    })
                    break
            }
