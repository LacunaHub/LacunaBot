...
export async function setLevelPrefix(self: Lacuna, server: ServerDocument, message: Message) {
    ...
    if (!mention) {
        await message.reply({
            content: `${self._emojis.ERROR} | ${self.translator.format(locale.activities['set-level'].texts.no_mention, `**${message.member.displayName}**`)}`
        })
    ...
    if (!set_level || set_level < 1 || set_level > 2500) {
        await message.reply({
            content: `${self._emojis.ERROR} | ${self.translator.format(locale.activities['set-level'].texts.no_level, `**${message.member.displayName}**`)}`
        })
    ...
    if (!level) {
        ...
        await self.db.users.updateOne(
            { _id: mention.id, 'activities.levels.guild_id': message.guildId },
            {
                $set: {
    ...
    await message.reply({
        content: `${self._emojis.OK} | ${self.translator.format(locale.activities['set-level'].texts.set_success, `**${message.member.displayName}**`)}`
    })
...
export async function setWalletBalancePrefix(self: Lacuna, server: ServerDocument, message: Message) {
    ...
    if (!mention) {
        await message.reply({
            content: `${self._emojis.ERROR} | ${self.translator.format(locale.activities['set-wallet-balance'].texts.no_mention, `**${message.member.displayName}**`)}`
        })
    ...
    if (!amount && typeof amount !== 'number') {
        await message.reply({
            content: `${self._emojis.ERROR} | ${self.translator.format(locale.activities['set-wallet-balance'].texts.no_amount, `**${message.member.displayName}**`)}`
        })
...
export async function addWalletBalancePrefix(self: Lacuna, server: ServerDocument, message: Message) {
    const locale = self.translator.locale(server.locale).commands
...
export async function resetWalletPrefix(self: Lacuna, server: ServerDocument, message: Message) {
    ...
    if (!activities.length) {
        await message.reply({
            content: `${self._emojis.ERROR} | ${self.translator.format(
                locale.activities['reset-all-wallets'].texts.nothing_to_reset,
                `**${message.member.displayName}**`
            )}`
        })
    ...
    if (member_id == 'all') {
        const row = new MessageActionRow().setComponents(
            new MessageButton().setCustomId('confirm').setStyle('DANGER').setLabel(locale.activities['reset-all-wallets'].texts.confirm),
            new MessageButton().setCustomId('cancel').setStyle('SECONDARY').setLabel(locale.activities['reset-all-wallets'].texts.cancel)
        )
        ...
        collector.on('collect', async i => {
            ...
            switch (i.customId) {
                ...
                case 'cancel':
                    await _message.edit({
                        content: `${self._emojis.OK} | ${self.translator.format(
                            locale.activities['reset-all-wallets'].texts.canceled,
                            `**${message.member.displayName}**`
                        )}`,
                        components: []
                    })
        ...
        await message.reply({
            content: `${self._emojis.OK} | ${self.translator.format(locale.activities['reset-wallet'].texts.reset_user, `**${message.member.displayName}**`)}`
        })
    }
...
export async function resetLevelPrefix(self: Lacuna, server: ServerDocument, message: Message) {
    ...
    if (!member && !member_id) {
        await message.reply({
            content: `${self._emojis.ERROR} | ${self.translator.format(locale.activities['reset-level'].texts.no_mention, `**${message.member.displayName}**`)}`
        })
    ...
    await message.reply({
        content: `${self._emojis.OK} | ${self.translator.format(locale.activities['reset-level'].texts.reset_success, `**${message.member.displayName}**`)}`
    })
